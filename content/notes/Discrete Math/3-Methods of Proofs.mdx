...
Is a proof method that can be used to denote that the proof holds regardless if the
cases being considered is less than the intented amount, ie. due to repitition.
...
Is a proof method in which when assuming that the statement is true leads it to becoming
not true or absurd.
